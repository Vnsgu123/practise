...
## You have to implement this task with the three available ##left
## modules for this task (numpy, opencv)                    ##left
##############################################################
import socket
import time
import os
...
def uturn(stream):
    ...
    # cv2.waitKey(2000)
    for frame in stream:
        print("uturn le raha hu kem cho  hguy")
        image1 = frame.array
...
def left(stream):
    ...
    R_MOTOR1.ChangeDutyCycle(29)
    L_MOTOR2.ChangeDutyCycle(33)
    stream1 = camera.capture_continuous(rawCapture, format="bgr", use_video_port=True)
...
def right(stream):
    ...
    L_MOTOR1.ChangeDutyCycle(24.5)
    R_MOTOR2.ChangeDutyCycle(26.5)
    stream1 = camera.capture_continuous(rawCapture, format="bgr", use_video_port=True)
    ...
    for frame in stream:
        ...
        # gausss11= cv2.cvtColor(image1, cv2.COLOR_BGR2HSV)
        # low_b = np.array([0, 0, 250])
        # high_b = np.array([180, 255, 255])
        # maskkkkk = cv2.inRange(gausss11, low_b, high_b)
        # out33 = cv2.bitwise_not(maskkkkk)
        ...
        if w1 > 10 and w1 < 300 and h1 > 27 and h1 < 520 and ter > count1 + 5 and count > count2 + 5:
            #cv2.waitKey(100)
            print("fnrefuyrgfygfyruyerfyurevyfuehiujoifjwu9hfywiyfhu34ty4ikr3h   ", w1)
...
def traversel(stream):
    ...
    for frame in stream:
        ...
        gausss1= cv2.cvtColor(gausss, cv2.COLOR_BGR2HSV)
        low_b = np.array([0, 0, 250])
        high_b = np.array([180, 255, 255])
        maskkkkk = cv2.inRange(gausss1, low_b, high_b)
        out33 = cv2.bitwise_not(maskkkkk)

        gray= cv2.cvtColor(gausss, cv2.COLOR_BGR2GRAY)
        #gray = cv2.cvtColor(gausss1, cv2.COLOR_HSV2GRAY)
        ret, thresh1 = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
        #grayyy = cv2.cvtColor(thresh1, cv2.COLOR_GRAY2HSV)
        ...
        #cv2.imshow('3', th3)
        cv2.imshow('thrOOOOPPOOOOOO',out33)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if message == "WAIT_5":
            wait()
            time.sleep(0.5)
        print('ha lya     0000000',message)

        kkkk = "NEXT"
        send_message_via_socket(client, kkkk)
        print("kvfgygmbfugkjbjhfhfsb")
    print("maitrey")
