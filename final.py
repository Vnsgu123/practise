...
def task_5_implementation(sim, config_img):
    """
    Purpose:
    ---
    ...
    traffic_signals, start_node, end_node = pb_theme.detect_all_nodes(
        config_img)
    ...
    while ll:
        ...
        # 		msg = pb_theme.receive_message_via_socket(connection_2)
        objectHandle = sim.getObject("/qr_plane_1")
...
def drop(drop_location, drop1, sim,connection_2):
    ...
    drop1 = drop1.split('_')
    print("DELIVERED: ", drop1[0], " ", drop1[1],
          " ", "AT", ' ', drop_location)
    op = drop1[0]+'off'
...
if __name__ == "__main__":
	...
	try:
		...
		image_filename = os.path.join(
		    os.getcwd(), message.lower(), "config_image.png")
		config_img = cv2.imread(image_filename)
	...
	task_5_implementation(sim,config_img)
